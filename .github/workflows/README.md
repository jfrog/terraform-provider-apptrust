...
After you approve, tests **do** run the customer SHA with Artifactory license secrets. That is required to test their code. Treat the approval as a trust decision: do not approve a PR that looks like it will dump env or exfiltrate credentials.
...
A PR that adds or changes resources should include a `CHANGELOG.md` bullet. That is a normal PR file. **Merging the PR lands that bullet.** CI does not write it.

### 2. CI “Tested on Artifactory …” heading (we cannot push to a fork)

The old job rewrote the version heading and `git push`ed to the PR branch. `GITHUB_TOKEN` cannot write to `customer/terraform-provider-apptrust`.

| PR type | After tests pass |
|---|---|
| Same-repo (`jfrog/terraform-provider-apptrust`) | CI still updates `CHANGELOG.md` and pushes, as before |
...
These must be done on `jfrog/terraform-provider-apptrust` after this lands on `main`:
