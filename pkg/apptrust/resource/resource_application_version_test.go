...
func TestAccApplicationVersion_basic(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(versionFqrn, "tag", "acc-test"),
					resource.TestCheckResourceAttrSet(versionFqrn, "id"),
				),
			},
			{
...
func TestAccApplicationVersion_updateTag(t *testing.T) {
	acctest.SkipIfNotAcc(t)
	acctest.PreCheck(t)
	...
	projectKey := acctest.AppTrustProjectKey1
	appKey := fmt.Sprintf("app-%d", id)
	version := fmt.Sprintf("2.0.%d", versionId)
